...
<?php 
  ...
  $ar_data[] = $_image; // ? 9
   ...
   if($_proses == "Simpan"){
    // data baru
    $sql = "INSERT INTO produk (kode, nama, harga_jual, harga_beli, stok, min_stok, deskripsi,kategori_produk_id, image) VALUES (?,?,?,?,?,?,?,?,?)";
  } else if ($_proses == "Update") {
     $ar_data[] = $_POST['idedit']; // ? 8
     $sql = "UPDATE produk SET kode=?, nama=?, harga_jual=?,harga_beli=?, stok=?, min_stok=?, deskripsi=?, kategori_produk_id=?, image=? WHERE id=?";
  }
   if(isset($sql)){
    $st = $dbh->prepare($sql);
    $st->execute($ar_data);
